...
export function GameLoadingModal({ isOpen, onClose, game, onLoadingComplete }: GameLoadingModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gradient-to-br from-slate-900/98 via-purple-900/95 to-slate-900/98 border border-purple-400/30 shadow-2xl backdrop-blur-xl text-white w-[90vw] max-w-sm sm:max-w-md rounded-2xl overflow-hidden z-50">
        {!showError ? (
          <>
            <DialogHeader>
              <div className="flex items-center justify-between p-4 sm:p-6 border-b border-purple-400/20">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg overflow-hidden border border-purple-400/30">
                    <img 
                      src={game.imageUrl} 
                      alt={game.name}
                  ...
                  <div>
                    <DialogTitle className="text-lg sm:text-xl font-bold text-white">{game.name}</DialogTitle>
            ...
            <div className="p-4 sm:p-6 text-center">
              ...
              <p className="text-gray-400 text-xs sm:text-sm mb-4">{loadingStage}</p>
          ...
          <>
            <div className="relative flex items-center justify-between p-4 sm:p-6 border-b border-purple-400/20">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg overflow-hidden border border-purple-400/30">
                  <img 
                    src={game.imageUrl} 
                ...
                <div>
                  <h2 className="text-lg sm:text-xl font-bold text-white">{game.name}</h2>
                  <p className="text-purple-300 text-xs sm:text-sm">
            ...
            <div className="p-4 sm:p-6">
              <div className="text-center space-y-4 sm:space-y-6">
                <div className="relative w-16 h-16 sm:w-20 sm:h-20 mx-auto">
                  <div className="w-full h-full bg-gradient-to-br from-red-500/20 to-red-600/20 rounded-full flex items-center justify-center border border-red-500/30">
                    <TriangleAlert className="h-8 w-8 sm:h-10 sm:w-10 text-red-400" />
                  </div>
                ...
                <div className="flex flex-col space-y-3 pt-2">
                  <Button
                    onClick={onClose}
                    className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-3 rounded-xl transition-all duration-300 shadow-lg hover:shadow-purple-500/25 text-sm sm:text-base"
                  >
                  ...
                  <Button
                    onClick={onClose}
                    variant="outline"
                    className="w-full border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white py-3 rounded-xl transition-all duration-300 text-sm sm:text-base"
                  >
